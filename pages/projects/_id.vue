          ...
          <div class="flex -mx-2">
            ...
            >
              <figure class="item-thumbnail cursor-pointer">
                <img
                  :src="$axios.defaults.baseURL + '/' + image.file_name"
                  @click="
                    changeImage($axios.defaults.baseURL + '/' + image.file_name)
                  "
                  alt=""
          ...
          >
            ...
            <div class="flex mt-3">
              <div class="w-1/4">
                <img
                  :src="
                    $axios.defaults.baseURL + '/' + campaign.data.user.avatar_file_name
                  "
                  alt=""
            ...
            <template v-if="this.$store.state.auth.user">
              <input
                type="number"
      ...
      <div class="flex justify-between items-center">
        <div class="w-full md:w-3/4 mr-6">
          ...
          <div class="flex progress-info mb-6">
            <div class="text-2xl">
              {{
                Math.round((campaign.data.current_amount / campaign.data.goal_amount) *
                100)
              }}%
            </div>
...
export default {
  async asyncData({ $axios, params }) {
    const campaign = await $axios.$get('/api/v1/campaign/' + params.slug)
    return { campaign }
  },
  data() {
    return {
  ...
  mounted() {
    this.default_image =
      this.$axios.defaults.baseURL + '/' + this.campaign.data.images
  },
}
</script>
